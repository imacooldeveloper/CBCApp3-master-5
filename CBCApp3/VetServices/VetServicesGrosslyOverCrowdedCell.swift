//
//  VetServicesGrosslyOverCrowdedCell.swift
//  CBCApp3
//
//  Created by Martin Gallardo on 10/31/18.
...
protocol VetServicesGrosslyOverCrowdedProtocol {
...
class CategoryCellVetGrosslyOverCrowded: UICollectionViewCell,UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,MFMailComposeViewControllerDelegate, UISearchBarDelegate  {
    
    
    private let cellID = "cellID"
    var grosslyOverCrowdedDelegate: VetServicesGrosslyOverCrowdedProtocol?
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell =  collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as? VetServicesGrosslyOverCrowdedCell {
            
            cell.grosslyOverCrowdedDelegate = self.grosslyOverCrowdedDelegate
            print(indexPath.row)
    ...
    func setupViews() {
        ...
        appCollectionViews.register(VetServicesGrosslyOverCrowdedCell.self, forCellWithReuseIdentifier: cellID)
...
class VetServicesGrosslyOverCrowdedCell: UICollectionViewCell,MFMailComposeViewControllerDelegate {
    ...
    var hourPlace: UITextField = {
        var text = UITextField()
        text.placeholder = "Hour"
        text.contentMode = .center
        text.borderStyle = .roundedRect
        text.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
        return text
    ...
    let overDueDiscription: UILabel = {
        let lab = UILabel()
        lab.text = "Action Required Separate and \n or wean cage within:"
        lab.numberOfLines = 0
    ...
    private func isEmthpy() {
        ...
        let overDueEmtpy = hourPlace.text?.count
        ...
        if overDueEmtpy == 0 {
            ...
            overDueLaby.text = "<br>Action Required Separate and or wean cage within \(hourPlace.text ?? "") Hours"
        }
    ...
    func configureMailController() -> MFMailComposeViewController {
        ...
        mailComposerVC.setMessageBody("To Whom it may Concern <br>Date: \(datePlaceHolder.text ?? "")<br>Cage Number \( search.text ?? "")<br>Room#: \(roomPlaceHolder.text ?? "")<br>Rack# \(rackNumPlace.text ?? "")<br>Location: \(cageLocationPlace.text ?? "")\(overDueLaby.text ?? "")\(extraNotes.text ?? "")<br><br><B> The above room contains Grossly Overcrowded cage(s). We request that you separate and/or wean your cages within 24/ 48 HOURS to comply with the current cage density guidelines set forth by regulatory agencies. If you are unable to comply Veterinary Services is required to act on your behalf and separate and/or wean any Grossly Overcrowded cage(s). Please note that there will be an associated fee for this service and that we do not provide a second notice for Grossly Overcrowded cages. (Definition of Grossly Overcrowded: More than 10 adults per cage; More than 20 adults and/or weanlings of greater than 21 days of age; More than 33 animals of any age, 20 or more mice in a cage when there are two litters more than 14 days apart in age with pups of various ages + adults).  Thank you, ", isHTML: true)
    ...
    @objc func handleEmail(){
        ...
        if MFMailComposeViewController.canSendMail() {
           ...
           grosslyOverCrowdedDelegate?.addGrowslyOverCrowdedEmail(view: mailComposeViewController)
    ...
    override init(frame: CGRect) {
        ...
        overDueDiscription.anchor(top: cageLocationLBL.bottomAnchor, bottom: nil, left: nil, right: nil, topPadding: 30, bottomPadding: 0, leftPadding: 20, rightPadding: 20, width: 0, height: 0)
        overDueDiscription.centerXAnchor.constraint(equalTo: taskView2.centerXAnchor).isActive = true
        taskView2.addSubview(hourLbl)
        hourLbl.anchor(top: overDueDiscription.bottomAnchor, bottom: nil, left: overDueDiscription.leadingAnchor, right: nil, topPadding: 10, bottomPadding: 0, leftPadding: 70, rightPadding: 0, width: 0, height: 0)
        taskView2.addSubview(hourPlace)
        hourPlace.anchor(top: hourLbl.topAnchor, bottom: hourLbl.bottomAnchor, left: hourLbl.trailingAnchor, right: nil, topPadding: 0, bottomPadding: 0, leftPadding: 10, rightPadding: 0, width: 0, height: 0)
        
        
        taskView2.addSubview(notePlaceHolder)
        notePlaceHolder.anchor(top: hourLbl.bottomAnchor, bottom: nil, left: taskView2.leadingAnchor, right: taskView2.trailingAnchor, topPadding: 50, bottomPadding: 0, leftPadding: 80, rightPadding: 80, width: 0, height: 90)
