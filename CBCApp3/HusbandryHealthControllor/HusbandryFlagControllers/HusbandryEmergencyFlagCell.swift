//
//  HusbandryEmergencyFlagCell.swift
//  CBCApp3
//
...
import UIKit
import MessageUI
protocol husbandryEmergencyProtocol {
...
class CategoryCellEmegrency: UICollectionViewCell,UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,MFMailComposeViewControllerDelegate, UISearchBarDelegate  {
    
    
    private let cellID = "cellID"
    var emergencyDelegate: husbandryEmergencyProtocol?
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell =  collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as? HusbandryEmergencyCell {
            
            cell.emergencyDelegate = self.emergencyDelegate
            print(indexPath.row)
    ...
    func setupViews() {
        ...
        appCollectionViews.register(HusbandryEmergencyCell.self, forCellWithReuseIdentifier: cellID)
...
class HusbandryEmergencyCell: UICollectionViewCell,MFMailComposeViewControllerDelegate, UISearchBarDelegate  {
    
    var emergencyDelegate: husbandryEmergencyProtocol?
    
    var taskView2: UIView = {
        var square = UIView()
        square.backgroundColor = #colorLiteral(red: 1, green: 0, blue: 0, alpha: 1)
        return square
    }()
    ...
    lazy var search: UISearchBar = {
        ...
        searchBar.barTintColor = #colorLiteral(red: 0.9981537461, green: 0.2163632512, blue: 0.25124681, alpha: 1)
        return searchBar
    }()
    ...
    func configureMailController() -> MFMailComposeViewController {
        ...
        mailComposerVC.setSubject("Emergency")
        mailComposerVC.setMessageBody("<BLOCKQUOTE>To Whom it may Concern <BLOCKQUOTE><BLOCKQUOTE>  Date:   \(datePlaceHolder.text ?? "")</BLOCKQUOTE><BLOCKQUOTE>  Cage Number \( search.text ?? "")</BLOCKQUOTE>Room#: \(roomPlaceHolder.text ?? "")<BLOCKQUOTE>Rack# \(rackNumPlace.text ?? "")</BLOCKQUOTE><BLOCKQUOTE></BLOCKQUOTE>Location:    \(cageLocationPlace.text ?? "")<BLOCKQUOTE>Extra Notes:  \(notePlaceHolder.text ?? "")</BLOCKQUOTE><BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE><b> Hey VetService Team i have a Emergency in this Cage", isHTML: true)
    ...
    @objc func handleEmail(){
        ...
        if MFMailComposeViewController.canSendMail() {
            ...
            if MFMailComposeViewController.canSendMail() {
               ...
               emergencyDelegate?.addHusbandryEmergencyView(view: mailComposeViewController)
