//
//  HusbandryOverCrowdedCell.swift
//  CBCApp3
//
//  Created by Martin Gallardo on 7/14/18.
...
protocol HusbandryOverCrowdedCellDelegate {
    func sendOverCrowdedEmail(view: UIViewController)
}

class CategoryCellOverCrowded: UICollectionViewCell,UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,MFMailComposeViewControllerDelegate, UISearchBarDelegate  {
    
    
    private let cellID = "cellID"
     var overCrowdedDelegate:  HusbandryOverCrowdedCellDelegate?
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell =  collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as? HusbandryOverCrowdedCell {
           
            cell.overCrowdedDelegate = self.overCrowdedDelegate
            print(indexPath.row)
    ...
    func setupViews() {
        ...
        appCollectionViews.register(HusbandryOverCrowdedCell.self, forCellWithReuseIdentifier: cellID)
...
class HusbandryOverCrowdedCell: UICollectionViewCell,MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    
    var overCrowdedDelegate:  HusbandryOverCrowdedCellDelegate?
    
    var healthStruck: HealthCheck? {
        didSet{
    ...
    lazy var search: UISearchBar = {
        ...
        searchBar.barTintColor = #colorLiteral(red: 0.968627451, green: 0.7882352941, blue: 0.5176470588, alpha: 1)
    ...
    func configureMailController() -> MFMailComposeViewController {
        ...
        mailComposerVC.setSubject("OverCrowded")
        mailComposerVC.setMessageBody("<BLOCKQUOTE>To Whom it may Concern <BLOCKQUOTE><BLOCKQUOTE>  Date:   \(datePlaceHolder.text ?? "")</BLOCKQUOTE><BLOCKQUOTE>  Cage Number \( search.text ?? "")</BLOCKQUOTE>Room#: \(roomPlaceHolder.text ?? "")<BLOCKQUOTE>Rack# \(rackNumPlace.text ?? "")</BLOCKQUOTE><BLOCKQUOTE></BLOCKQUOTE>Location:    \(cageLocationPlace.text ?? "")<BLOCKQUOTE></BLOCKQUOTE>Extra Notes:  \(notePlaceHolder.text ?? "")<BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE><b> Hey VetService Team i have a Over Crowded Cage", isHTML: true)
    ...
    @objc func handleEmail(){
        ...
        if MFMailComposeViewController.canSendMail() {
            ...
            if MFMailComposeViewController.canSendMail() {
                ...
                overCrowdedDelegate?.sendOverCrowdedEmail(view: mailComposeViewController)
