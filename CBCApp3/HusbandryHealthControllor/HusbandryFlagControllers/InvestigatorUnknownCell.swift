...
import UIKit
import MessageUI
protocol InvestigatorUnknownProtocol {
...
class CategoryCellUnknown: UICollectionViewCell,UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,MFMailComposeViewControllerDelegate, UISearchBarDelegate  {
    
    
    private let cellID = "cellID"
    var unknownDelegate: InvestigatorUnknownProtocol?
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell =  collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as? InvestigatorUnknownCell {
            
            cell.unknownDelegate = self.unknownDelegate
            print(indexPath.row)
    ...
    func setupViews() {
        ...
        appCollectionViews.register(InvestigatorUnknownCell.self, forCellWithReuseIdentifier: cellID)
...
class InvestigatorUnknownCell: UICollectionViewCell,MFMailComposeViewControllerDelegate,UISearchBarDelegate {
    ...
    func configureMailController() -> MFMailComposeViewController {
        ...
        mailComposerVC.setSubject("Investigator Unknown")
        mailComposerVC.setMessageBody("<BLOCKQUOTE>To Whom it may Concern <BLOCKQUOTE><BLOCKQUOTE>  Date:   \(datePlaceHolder.text ?? "")</BLOCKQUOTE><BLOCKQUOTE>  Cage Number \( search.text ?? "")</BLOCKQUOTE>Room#: \(roomPlaceHolder.text ?? "")<BLOCKQUOTE>Rack# \(rackNumPlace.text ?? "")</BLOCKQUOTE><BLOCKQUOTE></BLOCKQUOTE>Location:    \(cageLocationPlace.text ?? "")<BLOCKQUOTE>Extra Notes:  \(notePlaceHolder.text ?? "")</BLOCKQUOTE><BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE> <BLOCKQUOTE></BLOCKQUOTE><b> Hey VetService Team i have a Investigator Unknown Cage", isHTML: true)
    ...
    @objc func handleEmail(){
        ...
        if MFMailComposeViewController.canSendMail() {
            ...
            if MFMailComposeViewController.canSendMail() {
                unknownDelegate?.invesigatorAddEmail(view: mailComposeViewController)
    ...
    func setupUnknownViews() {
        ...
        datePlaceHolder.anchor(top: healthCheckLbl.topAnchor, bottom: healthCheckLbl.bottomAnchor, left: taskView2.leadingAnchor, right: healthCheckLbl.leadingAnchor, topPadding: 0, bottomPadding: 0, leftPadding: 50, rightPadding: 20, width: 0, height: 0)
...
//        NonDiscription.anchor(top: healthCheckLbl.bottomAnchor, bottom: nil, left: nil, right: nil, topPadding: 40, bottomPadding: 0, leftPadding: 0, rightPadding: 0, width: 0, height: 0)
