//
//  VetServicesLoginController.swift
//  CBCApp3
//
...
class VetServicesLoginController: UIViewController {
...
//        let tryUrl = "http://nanosoftech.com/store/user_check"
        
        
        let parameters: Parameters = [                               "ru_username":"mgallardo@rockefeller.edu",
                                                                     "ru_password":"rosasantanagallardo"
        ]
        //
//        let value: Parameters = ["email": "email",
//                                 "password":"password"]
        //
        
        
        Alamofire.request(url, method: .post, parameters: parameters, encoding: URLEncoding.default).responseJSON { (responseData) in
            debugPrint(responseData.data ?? "")
            
            print(responseData.result.value ?? "")
            if let err = responseData.error {
                print("FAILEDttttttttttttt:",err)
                return
            }
    ...
    @objc func handleLogin() {
        ...
        let vetServicesController = VetServicesFlagController(collectionViewLayout: flow)
        let navicon = CustomNavigation(rootViewController: vetServicesController)
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "Vet Services"
...
//    Error Domain=NSURLErrorDomain Code=-1202 "The certificate for this server is invalid. You might be connecting to a server that is pretending to be “preview2.rockefeller.edu”
//    
    func setipViews() {
        //
