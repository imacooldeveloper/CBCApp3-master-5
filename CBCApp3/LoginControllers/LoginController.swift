...
//  CBCApp3
//
//  Created by Martin Gallardo on 7/30/18.
...
class LoginController: UIViewController {
    ...
    @objc func handleLogin() {
        ...
        guard let floor = floorNum.text, floor.count > 0 else { return }
...
//        navigationController?.pushViewController(husbandryLoginController, animated: true)
    ...
    func setipViews() {
        ...
        let loginStack = UIStackView(arrangedSubviews: [emailText,floorNum,passwordText,loginBtn])
        loginStack.distribution = .fillEqually
        loginStack.axis = .vertical
        loginStack.spacing = 20
        view.addSubview(loginStack)
        loginStack.anchor(top: logoView.bottomAnchor, bottom: nil, left: view.leadingAnchor, right: view.trailingAnchor, topPadding: 40, bottomPadding: 0, leftPadding: 60, rightPadding: 60, width: 0, height: 0)
